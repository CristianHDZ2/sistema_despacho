...
$puedeEditar = !$solo_ver && 
               ($despacho ? !in_array($despacho['estado'], ['completado']) : true) && 
...
// Obtener precios especiales existentes (para liquidación)
$preciosEspeciales = [];
if ($despacho && in_array($despacho['estado'], ['pendiente_liquidacion', 'completado'])) {
    try {
        $stmt = $db->prepare("
            SELECT vpe.*, drd.producto_id
            FROM ventas_precios_especiales vpe
        ...
        foreach ($precios as $precio) {
            if (!isset($preciosEspeciales[$precio['producto_id']])) {
                $preciosEspeciales[$precio['producto_id']] = [];
            }
            $preciosEspeciales[$precio['producto_id']][] = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $puedeEditar) {
    ...
    if ($accion === 'crear_despacho') {
        // Crear nuevo despacho - CORREGIDO: Solo crear, no hacer nada más
        try {
            $stmt = $db->prepare("
    ...
    elseif ($accion === 'guardar_productos') {
        ...
        try {
            ...
            if ($evento === 'salida' && $productosGuardados === 0) {
                ...
                // Actualizar estado del despacho según el evento - CORREGIDO
                $nuevoEstado = '';
                switch ($evento) {
                    case 'salida':
                        $nuevoEstado = 'recarga';
                    ...
                    case 'segunda_recarga':
                        ...
                        break;
                    case 'retorno':
                        // Calcular ventas y preparar para liquidación - CORREGIDO
                        $stmt = $db->prepare("
                            UPDATE despacho_ruta_detalle 
                        ...
                        // CORREGIDO: Cambiar a pendiente_liquidacion, NO a completado
                ...
                // CORREGIDO: Redirigir a despachos.php después de guardar
    ...
    elseif ($accion === 'omitir_recarga') {
        ...
        try {
            ...
            if ($nuevoEstado) {
                ...
                // CORREGIDO: Redirigir a despachos.php después de omitir
                header('Location: despachos.php?fecha=' . $fecha);
                exit();
            }
    ...
    elseif ($accion === 'preview_liquidacion') {
        // Vista previa de liquidación con precios especiales - MEJORADO
        ...
        foreach ($detalles as $detalle) {
            ...
            if (isset($precios_especiales[$precio_especial_key]) && !empty($precios_especiales[$precio_especial_key])) {
                ...
                if (isset($precios_data['cantidad']) && is_array($precios_data['cantidad'])) {
                    foreach ($precios_data['cantidad'] as $index => $cantidad_esp) {
                        $cantidad_esp = (float)$cantidad_esp;
                        $precio_esp = (float)($precios_data['precio'][$index] ?? 0);
    ...
    elseif ($accion === 'liquidar_despacho') {
        // Procesar liquidación con precios especiales - MEJORADO
        $precios_especiales = $_POST['precios_especiales'] ?? [];
        ...
        try {
            ...
            // CORREGIDO: Marcar como completado (ahora sí)
            ...
            // CORREGIDO: Redirigir a despachos.php después de liquidar
            header('Location: despachos.php?fecha=' . $fecha);
            exit();
...
// Determinar evento actual y título - CORREGIDO con nuevo flujo
$eventoActual = '';
$tituloEvento = '';
$esObligatorio = true;
$puedeOmitir = false;
...
if ($despacho) {
    switch ($despacho['estado']) {
        ...
        case 'pendiente_liquidacion':
            $eventoActual = 'pendiente_liquidacion';
            $tituloEvento = 'Liquidar Despacho';
            break;
        case 'completado':
...
<body>
    ...
    <div class="content-area">
            ...
            <div class="container-fluid">
                ...
                <div class="navbar-nav ms-auto">
                    ...
                    <?php if ($_SESSION['tipo_usuario'] === 'admin' && $despacho): ?>
                        <button class="btn btn-outline-danger btn-sm" onclick="confirmarEliminarDespacho()">
        ...
        <div class="container-fluid p-4">
            ...
            <?php if ($despacho): ?>
                <!-- Progress Steps - CORREGIDO con nuevo flujo -->
                <div class="progress-steps">
                    <div class="progress-step <?php echo in_array($despacho['estado'], ['recarga', 'segunda_recarga', 'retorno', 'pendiente_liquidacion', 'completado']) ? 'completed' : 'active'; ?>">
                        <div class="step-circle">1</div>
                        <small>Salida</small>
                    </div>
                    <div class="progress-step <?php echo in_array($despacho['estado'], ['segunda_recarga', 'retorno', 'pendiente_liquidacion', 'completado']) ? 'completed' : ($despacho['estado'] === 'recarga' ? 'active' : ''); ?>">
                        <div class="step-circle">2</div>
                        <small>1ra Recarga</small>
                    </div>
                    <div class="progress-step <?php echo in_array($despacho['estado'], ['retorno', 'pendiente_liquidacion', 'completado']) ? 'completed' : ($despacho['estado'] === 'segunda_recarga' ? 'active' : ''); ?>">
                        <div class="step-circle">3</div>
                        <small>2da Recarga</small>
                    </div>
                    <div class="progress-step <?php echo in_array($despacho['estado'], ['pendiente_liquidacion', 'completado']) ? 'completed' : ($despacho['estado'] === 'retorno' ? 'active' : ''); ?>">
                ...
                <?php elseif ($eventoActual === 'pendiente_liquidacion'): ?>
                    <!-- NUEVA SECCIÓN: Liquidación con precios especiales -->
                    <div class="card">
                        <div class="card-header bg-warning text-dark">
                            <h5 class="mb-0">
                                <i class="fas fa-calculator me-2"></i>Liquidar Despacho
                            </h5>
                        </div>
                        <div class="card-body">
                            <div class="alert alert-info">
                                ...
                                <strong>Puede agregar precios especiales para cualquier producto.</strong>
                            </div>
                            ...
                            <form method="POST" action="" id="formLiquidacion">
                                ...
                                <?php if (!empty($detalles)): ?>
                                    <?php foreach ($detalles as $detalle): ?>
                                        <div class="card mb-3">
                                            <div class="card-header bg-light">
                                                <div class="row align-items-center">
                                                    <div class="col">
                                                        <h6 class="mb-0"><?php echo htmlspecialchars($detalle['producto_nombre']); ?></h6>
                                                        <small class="text-muted">
                                                            Precio normal: <?php echo formatearDinero($detalle['precio_unitario']); ?>
                                                            <?php if ($detalle['usa_formula']): ?>
                                                                <span class="badge bg-info ms-2">Usa Fórmula</span>
                                                            <?php endif; ?>
                                                        </small>
                                                    </div>
                                                    <div class="col-auto">
                                                        <span class="badge bg-primary">
                                                            Vendido: <?php echo formatearNumero($detalle['ventas_calculadas']); ?>
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="card-body">
                                                <div class="precio-especial-section">
                                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                                        <h6 class="mb-0">
                                                            <i class="fas fa-tags me-2"></i>Precios Especiales (Opcional)
                                                        </h6>
                                                    ...
                                                    <div id="precios_especiales_<?php echo $detalle['producto_id']; ?>">
                                                        ...
                                                        if (isset($preciosEspeciales[$detalle['producto_id']])) {
                                                            foreach ($preciosEspeciales[$detalle['producto_id']] as $index => $precio) {
                                                        ?>
                                                            <div class="row mb-2 precio-especial-row">
                                                                <div class="col-4">
                                                                    <input type="number" class="form-control form-control-sm" 
                                                                           name="precios_especiales[<?php echo $detalle['producto_id']; ?>][cantidad][]" 
                                                                           placeholder="Cantidad" step="0.5" min="0" 
                                                                           value="<?php echo $precio['cantidad']; ?>">
                                                                </div>
                                                                <div class="col-4">
                                                                    <input type="number" class="form-control form-control-sm" 
                                                                           name="precios_especiales[<?php echo $detalle['producto_id']; ?>][precio][]" 
                                                    ...
                                                    <small class="text-muted">
                                                        ...
                                                        Agregue precios especiales si vendió algunas unidades a precio diferente al normal.
                                                        El resto se calculará automáticamente con el precio normal.
                                                    </small>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="row mt-4">
                                        <div class="col-12">
                                            <button type="button" class="btn btn-info me-2" onclick="previsualizarLiquidacion()">
                                                <i class="fas fa-eye me-2"></i>Vista Previa
                                            </button>
                                            <button type="submit" class="btn btn-success">
                                                <i class="fas fa-check me-2"></i>Liquidar y Completar
                                            </button>
                                        </div>
                                    </div>
                                <?php endif; ?>
